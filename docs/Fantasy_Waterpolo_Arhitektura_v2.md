...
15. User accounts, auth, and the fantasy-team layer (leagues/fantasy_teams/rosters/lineups/transfers) — nothing built yet; this is the actual fantasy game, still entirely unstarted. Depends on deciding an auth approach (JWT vs OAuth-only, per the v1 doc's original plan) before starting.
16. Frontend (Next.js, per Section 5.3) — not started.
