...
12. Coach scraping and scoring — deferred by choice, not blocked: the project owner will supply real coach names alongside player positions in one reference file. Until then, every club has a generic placeholder `Coach` row (`db_writer.create_placeholder_coaches_for_competition`, name suffixed "— trener TBD", base 7 CR) so team-building isn't blocked by the missing coach slot. `coach_points()` exists in `scoring/engine.py`, ready for when real `coach_stats` exist.
13. Resolve `players.position` (OT/CF/CB) — still null for every field player; pending the same reference file as coaches.
14. ~~Build the API layer~~ — done: read-only sports-data endpoints (`backend/app/routers/`) for competitions, standings (computed from finished matches, not stored), players (list + detail with season totals and price history), matches (with box score, reusing `scoring.engine.player_raw_points` so the numbers match `fantasy_scores` exactly), and per-matchday top performers. Verified live against the real database — e.g. a goalkeeper's API-computed raw_points (12.0: 9 saves × 1.5 − 7 conceded × 0.5 + 2 steals × 1) matched the stored `fantasy_scores` value exactly. No auth or fantasy-team layer yet (see next items).
15. ~~User accounts and auth~~ — done, scoped down: email+password only (`backend/app/routers/auth.py`), bcrypt password hashing, a single 7-day JWT access token (no refresh-token pair, no Google OAuth yet -- both were in the original v1 doc's plan, deferred since neither is needed to test the core mechanics). Verified live: register, `/me`, login, wrong password (401), missing token (401), duplicate email (409) all behave correctly.
16. ~~The fantasy-team layer: creation, roster ownership, transfers~~ — done (`backend/app/routers/teams.py`), scoped to what's actually possible without position data: `POST /api/teams` validates roster size (11 players + 1 coach) and budget (≤100 credits) and auto-provisions the one global `League` per (competition, season) on first use; `POST /api/teams/{id}/transfers` is a locked, transactional buy/sell (`SELECT ... FOR UPDATE` on the team row, per the concurrency concern in Section 5.4) with no transfer limit (per OQ-4) — which also makes the wildcard mechanic vestigial, since it only ever existed to bypass a limit that no longer exists, so it isn't implemented. Verified live: team creation, duplicate-team rejection (409), a real transfer with exact balance arithmetic (49.00 → 42.86 credits, matching sell/buy prices precisely), "already on team" rejection, and transfer_history logging.
    - **Still missing / explicitly out of scope here:** lineup management (formation, bench/captain, per-matchday scoring) — blocked on `players.position`, which is null for every player. `transfer_history.matchday_id` is NOT NULL in the schema but there's no live season with a genuinely "current" matchday yet (every scraped season is fully finished) — `_get_current_matchday_id` falls back to the season's last matchday as a placeholder; revisit once a live/ongoing season exists.
17. Frontend (Next.js, per Section 5.3) — not started.
