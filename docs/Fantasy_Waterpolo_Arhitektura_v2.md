...
16. The fantasy-team layer itself (leagues/fantasy_teams/rosters/lineups/transfers) — still entirely unstarted; this is the actual fantasy game. Partially blocked on `players.position`: roster validation can check budget (sum of costs ≤ 100) and slot count (12) today, but not the exact per-formation GK/OT/CF/CB counts until position data arrives. The one global `leagues` row per (competition, season) also hasn't been provisioned yet -- the `leagues` table is still empty.
